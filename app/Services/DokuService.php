...
class DokuService
{
    ...
    public function __construct()
    {
        ...
        if (!file_exists($privateKeyPath)) {
            throw new \RuntimeException('Private key tidak ditemukan: ' . $privateKeyPath);
        }
    ...
    public function createQrisPayment(string $reference, int $amount): array
    {
        ...
        try {
            $response = Http::timeout(15)->withHeaders([
                ...
                'X-SIGNATURE'    => $signature,
                'CHANNEL-ID'     => 'H2H',
            ])->withBody($bodyJson, 'application/json')->post($this->baseUrl . $endpoint);
    ...
    public function refundQrisPayment(
        ...
        $timestamp = now()->utc()->format('Y-m-d\TH:i:s\Z');
        $endpoint = '/snap-adapter/b2b/v1.0/qr/qr-mpm-refund';

        $body = [
            'originalPartnerReferenceNo' => $originalPartnerReferenceNo,
            'originalReferenceNo'        => $originalReferenceNo,
            'partnerRefundNo'            => $refundPartnerReferenceNo,
            'merchantId'                 => config('services.doku.merchant_id'),
            'refundAmount' => [
