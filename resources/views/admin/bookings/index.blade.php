...
@section('content')
    <div class="space-y-6">
            ...
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('admin.bookings.create') }}"
                       class="inline-flex items-center gap-2 rounded-xl border-2 border-gray-900 bg-brand px-4 py-2.5 text-xs font-black uppercase text-white shadow-[3px_3px_0px_0px_rgba(17,24,39,1)] transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[5px_5px_0px_0px_rgba(17,24,39,1)] active:translate-x-0 active:translate-y-0 active:shadow-none">
                    ...
                    <a href="{{ $exportUrl ?? '#' }}"
                       class="inline-flex items-center gap-2 rounded-xl border-2 border-gray-900 bg-white px-4 py-2.5 text-xs font-black uppercase text-brand shadow-[3px_3px_0px_0px_rgba(17,24,39,1)] transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-brand/5 hover:shadow-[5px_5px_0px_0px_rgba(17,24,39,1)] active:translate-x-0 active:translate-y-0 active:shadow-none">
                        <svg class="h-4 w-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($bookings as $booking)
                            @php
                                ...
                                $statusLabel = $statusOptions[$booking->status] ?? ucfirst(str_replace('_', ' ', $booking->status));
                            @endphp
                            <tr class="bg-white hover:bg-gray-50">
                                ...
                                <td class="px-5 py-4 align-top">
                                    <div class="font-semibold text-gray-900">Rp {{ number_format($booking->total_amount, 0, ',', '.') }}</div>
                                    @if ($booking->outstanding_amount > 0)
                                        <div class="mt-1 text-xs font-semibold text-amber-600">Sisa: Rp {{ number_format($booking->outstanding_amount, 0, ',', '.') }}</div>
                                    @endif
                                </td>
                                <td class="px-5 py-4 align-top">
        ...
        <div class="flex flex-col justify-between gap-3 text-xs font-bold text-gray-600 sm:flex-row sm:items-center">
            <p>
                Showing {{ $bookings->firstItem() ?? 0 }} to {{ $bookings->lastItem() ?? 0 }} of {{ $bookingCount }} results
            </p>
    ...
    @foreach ($bookings as $booking)
        <!-- Dropdown Menu -->
        <div id="booking-actions-{{ $booking->id }}" class="z-30 hidden divide-y divide-gray-100 rounded-lg border border-gray-200 bg-white shadow-lg">
            <ul class="p-2 text-sm text-gray-700">
                <li>
                    <button type="button" 
                            data-modal-target="detailBookingModal-{{ $booking->id }}" 
                            data-modal-toggle="detailBookingModal-{{ $booking->id }}" 
                ...
                <li>
                    <a href="{{ route('admin.bookings.edit', $booking) }}" class="block w-full rounded px-3 py-2 text-left hover:bg-gray-100">
                        Edit Booking
                    </a>
                </li>
                <li>
                    <button type="button" 
                            data-modal-target="statusModal-{{ $booking->id }}" 
                            data-modal-toggle="statusModal-{{ $booking->id }}" 
                            data-dropdown-hide="booking-actions-{{ $booking->id }}"
                ...
                @if ($booking->status !== 'cancelled')
                    <li>
                        <button type="button" 
                                data-modal-target="cancelModal-{{ $booking->id }}" 
                                data-modal-toggle="cancelModal-{{ $booking->id }}" 
                                data-dropdown-hide="booking-actions-{{ $booking->id }}"
                                class="w-full rounded px-3 py-2 text-left text-red-600 hover:bg-red-50">
                            Cancel Booking
                        </button>
                    </li>
                @endif
        ...
        <!-- Detail Modal -->
        <div id="detailBookingModal-{{ $booking->id }}" tabindex="-1" aria-hidden="true" class="fixed inset-0 z-50 hidden h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden bg-gray-900/50 p-4 backdrop-blur-xs">
            <div class="relative max-h-full w-full max-w-2xl">
                <div class="relative rounded-xl bg-white shadow">
                    <div class="flex items-center justify-between border-b border-gray-200 p-4 md:p-5">
                        <h3 class="font-montserrat text-xl font-semibold text-gray-900">Booking #BK-{{ $booking->id }}</h3>
                        <button type="button" data-modal-hide="detailBookingModal-{{ $booking->id }}" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100">&times;</button>
                    </div>
                    <div class="grid gap-5 p-4 text-sm md:grid-cols-2 md:p-5">
                        <div>
                            <h4 class="mb-2 font-semibold text-gray-900">Customer</h4>
                            <p class="font-medium text-gray-800">{{ $booking->name }}</p>
                            <p class="text-gray-500">{{ $booking->phone }}</p>
                        </div>
                        <div>
                            ...
                            <div class="divide-y divide-gray-100 rounded-lg border border-gray-200">
                                @forelse ($booking->items as $item)
                                    <div class="flex justify-between gap-3 p-3">
                                        <span>{{ $item->service_name_snapshot ?? $item->product_name_snapshot ?? $item->service?->name ?? $item->product?->name ?? 'Item' }} x{{ $item->qty }}</span>
                                        <span class="font-semibold text-gray-900">Rp {{ number_format($item->price_snapshot * $item->qty, 0, ',', '.') }}</span>
                                    </div>
                                @empty
                                    <div class="p-3 text-gray-500">No items recorded.</div>
                                @endforelse
                            </div>
                        </div>
                        <div>
                            <h4 class="mb-2 font-semibold text-gray-900">Note</h4>
                            <p class="text-gray-600">{{ $booking->description ?: 'No special notes.' }}</p>
                        </div>
                        <div>
                            <h4 class="mb-2 font-semibold text-gray-900">Payment Summary</h4>
                            <p>Total: <strong>Rp {{ number_format($booking->total_amount, 0, ',', '.') }}</strong></p>
                            <p>Outstanding: <strong>Rp {{ number_format($booking->outstanding_amount, 0, ',', '.') }}</strong></p>
                            <p class="capitalize text-gray-500">Type: {{ strtoupper($booking->payment_type) }}</p>
                        </div>
                        <div class="md:col-span-2">
                            <h4 class="mb-2 font-semibold text-gray-900">Payment Info</h4>
                            @if ($booking->payment)
                                <div class="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 p-3">
                                    <div>
                                        <p class="font-medium capitalize text-gray-800">
                                            {{ str_replace('_', ' ', $booking->payment->purpose ?? 'payment') }} &bull; {{ strtoupper(str_replace('_', ' ', $booking->payment->method ?? '-')) }}
                                        </p>
                                        <p class="text-xs text-gray-500">
                                            Status: <span class="font-semibold text-gray-700">{{ ucfirst($booking->payment->status) }}</span>
                                        </p>
                                    </div>
                                    <span class="text-sm font-bold text-gray-900">Rp {{ number_format($booking->payment->amount, 0, ',', '.') }}</span>
                                </div>
                            @else
                                <p class="text-gray-500">No payment record.</p>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <!-- Status Update Modal -->
        <div id="statusModal-{{ $booking->id }}" tabindex="-1" aria-hidden="true" class="fixed inset-0 z-50 hidden h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden bg-gray-900/50 p-4 backdrop-blur-xs">
            <div class="relative w-full max-w-md">
                <div class="relative rounded-xl bg-white shadow-lg">
                    <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
                        <h3 class="font-bold text-gray-900">Update Booking Status #BK-{{ $booking->id }}</h3>
                        <button type="button" data-modal-hide="statusModal-{{ $booking->id }}" class="rounded-lg p-1 text-gray-400 hover:text-gray-600 transition">
                    ...
                    <form method="POST" action="{{ route('admin.bookings.update-status', $booking) }}" class="space-y-4 p-5">
                        @csrf
                        @method('PATCH')

                        <div>
                            <label for="status-{{ $booking->id }}" class="mb-1.5 block text-sm font-medium text-gray-700">
                        ...
                        <div class="flex items-center justify-end gap-2 pt-2">
                            <button type="button" data-modal-hide="statusModal-{{ $booking->id }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                                Cancel
                            </button>
