...
@section('content')
    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
            <h2 class="font-league text-4xl uppercase text-gray-900">Services</h2>
            <p class="mt-1 text-sm text-gray-500">Manage haircut, treatment, and other barber services.</p>
        </div>
        <button
            type="button"
            ...
            class="flex items-center gap-2 rounded-base border border-transparent bg-brand px-4 py-2.5 text-sm font-medium text-white shadow-xs hover:bg-brand-strong focus:outline-none focus:ring-4 focus:ring-brand-medium"
        >
                        ...
                        <tr class="bg-white hover:bg-gray-50">
                            <td class="px-6 py-4">
                                ...
                                @if ($service->description)
                                    <div class="mt-1 max-w-md truncate text-xs text-gray-500">{{ $service->description }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 capitalize">{{ $service->category }}</td>
                            <td class="px-6 py-4">Rp {{ number_format($service->price, 0, ',', '.') }}</td>
                            <td class="px-6 py-4">
                                <span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $service->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
