<section aria-labelledby="payment-title">
    <h1 id="payment-title" class="sr-only">Pembayaran</h1>

    <div class="flex flex-col md:flex-row gap-4">
        <!-- PILIH TIPE PEMBAYARAN -->
        <div class="w-full md:w-1/3 rounded-xl bg-white border border-gray-200 p-6">
            <span class="block pb-4 text-xl font-montserrat font-bold tracking-tight text-primary">PILIH TIPE PEMBAYARAN *</span>
            <div class="flex flex-col gap-3">
                <!-- Opsi 1: DP (Down Payment) -->
                <div @click="if (!paymentTypeConfirmed && paymentData?.status !== 'paid') paymentType = 'DP'" :class="paymentType === 'DP' ? 'border-primary' : 'border-default bg-neutral-primary-soft'" class="border border-default rounded-2xl p-4 cursor-pointer">
                ...
                <!-- Opsi 2: Pelunasan Langsung (Full Payment) -->
                <div @click="if (!paymentTypeConfirmed && paymentData?.status !== 'paid') paymentType = 'Full'" :class="paymentType === 'Full' ? 'border-primary' : 'border-default bg-neutral-primary-soft'" class="border border-default rounded-2xl p-4 cursor-pointer">
                    <div class="flex items-start">
                        ...
                        <label for="bordered-radio-2" class="w-full select-none ms-3 cursor-pointer flex flex-col">
                            ...
                            <span class="text-2xs text-gray-500 font-normal">Bayar seluruh total biaya sekarang</span>
                        </label>
                    </div>
                ...
                >
                    ...
                    <span x-show="paymentState !== 'loading' && paymentState !== 'ready'">Konfirmasi Tipe Pembayaran</span>

                    <!-- Saat Proses Simpan Pemesanan & Generate QRIS -->
                    <span x-show="paymentState === 'loading'">Memproses Pemesanan...</span>

                    <!-- Saat Berhasil & QRIS Sudah Siap -->
                    <span x-show="paymentState === 'ready'">Tipe Pembayaran Dikonfirmasi ✓</span>
                </button>
            </div>
        </div>
        ...
        <div class="w-full md:w-1/3 rounded-xl bg-white border border-gray-200 p-6">
            <span class="block pb-4 text-xl font-montserrat font-bold tracking-tight text-primary">PEMBAYARAN</span>
            <div class="flex flex-col items-center gap-4 text-center w-full">

                <!-- Placeholder Saat QRIS Belum Di-generate -->
                <div x-show="!paymentData?.qrContent" class="flex flex-col py-5 lg:pt-32 items-center justify-center w-full text-center">
                    ...
                    <p class="font-semibold text-gray-800">Kode Pembayaran QRIS</p>
                    <p class="text-xs text-gray-500 mt-1 max-w-xs">Pilih tipe pembayaran Anda di atas lalu klik konfirmasi untuk memuat kode QRIS.</p>
                    
                    <!-- Pesan Error -->
                    <p x-show="paymentError" x-text="paymentError" class="text-sm text-red-600 mt-2"></p>
                </div>
                ...
                <template x-if="paymentData?.qrContent && paymentData?.status !== 'paid'">
                    <div class="flex flex-col items-center justify-center gap-3 w-full text-center">
                        <div class="flex gap-3 items-center">
                            <img src="{{ asset("images/qris.png") }}" alt="Kode Pembayaran QRIS" class="h-10 w-auto object-contain">
                        ...
                        <div class="w-full py-3 flex flex-col items-center justify-center border rounded-xl border-gray-200">
                            <canvas x-ref="qrisCanvas" aria-label="Kode pembayaran DOKU QRIS" class="h-56 w-56"></canvas>
                             <!-- Tautan Pembayaran Alternative -->
                            <a
                                ...
                                Lihat halaman pembayaran pemesanan
                            </a>
                        </div>
                ...
                <div x-show="paymentData?.status === 'paid'" class="w-full flex flex-col justify-center items-center rounded-xl bg-green-50 px-4 py-4 text-center text-green-700">
                    ...
                    <p class="font-semibold">Pembayaran berhasil diterima</p>
            ...
            <template x-for="(guest, guestIndex) in guests" :key="guestIndex">
                <div class="flex flex-col gap-2.5 py-4 border-b border-gray-200 last:border-b-0">
                    <div class="flex items-center justify-between gap-2.5">
                        <div class="text-md font-bold text-gray-900 truncate" x-text="guest.name || 'Nama Pelanggan'"></div>
                    </div>
    ...
    <div class="flex justify-between mt-4">
        ...
        <button
            type="button"
            class="btn rounded-xl bg-brand text-white disabled:cursor-not-allowed disabled:opacity-50 flex items-center gap-2 px-4 py-2 text-sm font-semibold transition-colors"
            :disabled="paymentData?.status !== 'paid'"
